...
export class FileStorageService implements OnModuleInit {
  ...
  private async saveImgs(img: Buffer, name: string) {
    await this.s3
      .upload({ Bucket: BUCKET_NAME, Key: name, Body: img })
      .promise();
  }
  ...
  async saveToDist(pictureList: CrawledPicture[]): Promise<Picture[]> {
    ...
    return Promise.all(
      imgList.map(async ({ img, name, picture }): Promise<Picture> => {
        this.logger.info('Gen gallery and thumbnail imgs', {
          url: picture.originalImgUrl,
          name,
        });
