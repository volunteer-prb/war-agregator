...
async function getDate(post: puppeteer.ElementHandle<Element>) {
  ...
  const dateStringValue: string = await timeTag.evaluate(v => v.getAttribute('datetime'))
...
@Injectable()
export class TelegramService implements DataIngestionServices, OnModuleInit, OnModuleDestroy {
  ...
  private async updatePage() {
    await this.page.goto('https://t.me/s/truexanewsua', { waitUntil: 'domcontentloaded' })
  }
